...
class scene {
  ...
  struct script_handle {
    ...
    using on_create_fn_type = void(*)(scene&, std::type_index);
    using on_destroy_fn_type = void(*)(scene&, std::type_index);
    using on_update_fn_type = void(*)(scene&, std::type_index, std::float_t);
...
private:
  ...
  template<typename Type>
  static auto _on_create(scene& scene, std::type_index type) -> void;

  template<typename Type>
  static auto _on_destroy(scene& scene, std::type_index type) -> void;

  template<typename Type>
  static auto _on_update(scene& scene, std::type_index type, std::float_t delta_time) -> void;

  template<typename Type>
  static auto _deleter(void* ptr) -> void;

  std::unordered_map<std::type_index, script_handle> _scripts{};
